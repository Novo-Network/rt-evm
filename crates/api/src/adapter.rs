...
use rt_evm_model::{
    ...
    types::{
        Account, BigEndianHash, Block, BlockNumber, ExecutorContext, Hash, Header, Proposal,
        Receipt, SignedTransaction, TxResp, H160, MAX_BLOCK_GAS_LIMIT, MIN_GAS_PRICE, NIL_HASH,
        U256, WORLD_STATE_META_KEY,
    },
};
...
impl DefaultAPIAdapter {
    pub fn new(mempool: Arc<Mempool>, trie_db: Arc<MptStore>, storage: Arc<Storage>) -> Self {
        Self {
            mempool,
            trie_db,
            storage,
    ...
    pub fn evm_backend(&self, number: Option<BlockNumber>) -> Result<RTEvmExecutorAdapter> {
        let block = self
...
impl APIAdapter for DefaultAPIAdapter {
    ...
    fn get_account(&self, address: H160, number: Option<BlockNumber>) -> Result<Account> {
        ...
        let account = match state.get(address.as_bytes()) {
    ...
    fn evm_call(
        ...
        let backend =
            RTEvmExecutorAdapter::from_root(state_root, &self.trie_db, &self.storage, exec_ctx)?;
    ...
    fn get_storage_at(&self, address: H160, position: U256, state_root: Hash) -> Result<Vec<u8>> {
